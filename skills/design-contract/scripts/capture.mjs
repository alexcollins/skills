...
const rows = written.map((w) => `| \`${w.file}\` | ${w.url.replace(config.base, '') || '/'} |${w.overflow ? ' ⚠️ scrolls sideways |' : ' |'}`).join('\n');
writeFileSync(
  join(DIR, 'README.md'),
  `# Visual reference

Full-page captures of every page template at ${config.widths.join(' and ')} wide, taken with
